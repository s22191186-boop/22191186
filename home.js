...
function updateLegend(list){
  ...
  const distLabel = (distMode === 'station')
    ? '並び順：最寄駅に近い順'
  ...
  const chips = keys.map(k => {
    const c = counter[k];
    const color = pinColor(k);
    return `<span style="display:inline-flex;align-items:center;gap:6px;background:#fff;border:1px solid #e5e7eb;padding:2px 8px;border-radius:999px;margin:2px 6px 0 0;font-size:12px;">
              <i style="display:inline-block;width:14px;height:14px;border-radius:50%;background:${color};border:1px solid #fff;box-shadow:0 0 0 1px rgba(0,0,0,.08) inset"></i>
              ${esc(k)}<b style="margin-left:4px">${c}</b>
            </span>`;
  }).join('');

  legendEl.innerHTML = `
    <div style="font-weight:700;color:#0f6c82;margin-bottom:2px">選択中：${esc(areaSel)}／${esc(citySel)}</div>
    <div style="color:#3a7a83;margin-bottom:4px">${distLabel} / 件数：<b>${total}</b></div>
    <div>${chips}</div>
  `;

  // 摆到缩放控件右侧 0.3cm，等高且宽=3倍
