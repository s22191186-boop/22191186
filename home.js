...
function pinColor(t){
  return TYPE_PIN[norm(t)] || TYPE_PIN.default;
}
function escape(s){
...
/**
 * list: 当前要渲染到地图的设施数组（render 调用时传入）
 * 功能: 显示 ①选择区域 ②排序方式 ③件数 ④各类型颜色与数量
 */
function updateLegend(list){
  ...
  const chips = keys.map(k => {
    ...
    return `<span class="chip"><i style="background:${color}"></i>${escape(k)}<b style="margin-left:6px">${c}</b></span>`;
  }).join('');

  legendEl.innerHTML = `
    <div class="legend-title">選択中：${escape(areaSel)}／${escape(citySel)}</div>
    <div class="legend-sub">${distLabel}　/　件数：<b>${total}</b></div>
