...
async function loadOne(url) {
  ...
  for (const f of (gj.features || [])) {
    ...
    // 名称／住所（補正：P27_004＝名称、P27_005＝住所。P05等にも対応）
    ...
    // TYPE は QGIS 側で日本語カテゴリに正規化済み（03002→「博物館」など。処理後の文字列を使用）
    let uiType = (p.TYPE || p.Type || '').toString().trim();
    // 大分類の統合：031xx →「スポーツ施設」（不要であれば削除可）
...
async function nearestStationMode(base){
  const stRes = await fetch('N02-24_Station.geojson');  // 駅データ（あなたのファイル）
