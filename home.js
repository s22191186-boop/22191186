  ...
  const rows = keys.map(k => {
    ...
    return `
      <div class="lg-row"
        ...
        <span class="lg-city"  style="font-weight:700;color:#0f6c82;">${esc(cityLabel)}</span>
        <span class="lg-sep">・</span>
        <i class="lg-dot"
           style="width:10px;height:10px;border-radius:50%;
                  background:${color};display:inline-block;
                  border:1px solid #fff;box-shadow:0 0 0 1px rgba(0,0,0,.08) inset"></i>
        <span class="lg-type"  style="color:#355d66;">${esc(k)}</span>
        <span class="lg-count" style="margin-left:auto;font-weight:700;">件数：${cnt}</span>
      </div>`;
  }).join('');

  legend.innerHTML =
    `<div class="lg-wrap"
          style="display:flex;flex-direction:column;gap:4px;font-size:12px;line-height:1.25;">
        ${rows}
     </div>`;

  // 重新贴齐到缩放控件右侧（你的 positionLegendNextToZoom 已经在文件里）
  positionLegendNextToZoom();

  // 如需要在固定高度里滚动而不是被裁切，可放开下一行：
